...
public class Property {
	...
	private Attribute  annoAttribute;
...
//	private List<ValidateInfo> validates;
	...
	public static Property createProperty(Class<?> clazz,Field field) {
		...
		return new Property(clazz,field,attribute,column);
	}
	 ...
	 * @param field
	 */
	private Property(Class<?> clazz,Field field,Attribute attribute,Column annoColumn) {
		this.clazz = clazz;
		this.field = field;
		this.fieldType = this.field.getType();
		this.annoColumn = annoColumn;
		this.annoAttribute = attribute;
		if(this.annoColumn!=null) {
			this.isDB = true;
		}
		this.name = this.field.getName();
		this.title = this.annoAttribute.title();
		this.remark = this.annoAttribute.remark();
		this.timeFormat = this.annoAttribute.timeFormat();
		this.length= this.annoAttribute.length();
		this.precision= this.annoAttribute.precision();
		initDBColumn();
	}
	...
	private void initDBColumn() {
		if(this.isDB) {
			this.columnName = this.annoColumn.columnName();
			...
			if(this.fieldType.isAssignableFrom(Integer.class)||this.fieldType.isAssignableFrom(int.class)) {
				...
				//单精度
				this.columnDataType = ColumnDataType.DECIMAL;
				if(this.precision==0) {
					this.precision = 2;
				}
				...
				//日期
				this.columnDataType = ColumnDataType.DATE;
			}else if(this.fieldType.isAssignableFrom(Timestamp.class)) {
				//时间
				this.columnDataType = ColumnDataType.TIME;
				...
				//字节.二进制的数据
				this.columnDataType = ColumnDataType.BINARY;
			}
	...
	public Attribute getAnnoAttribute() {
		return annoAttribute;
	}

	public void setAnnoAttribute(Attribute annoAttribute) {
		this.annoAttribute = annoAttribute;
	}
