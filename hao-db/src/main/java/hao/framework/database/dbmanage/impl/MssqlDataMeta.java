...
public class MssqlDataMeta extends DataMeta {
	...
	public List<String> createTableDDL(EntityInfo entity) {
		...
		if(entity.getTableTitle()!=null&&!entity.getTableTitle().equals("")){
			list.add("EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'"+entity.getTableTitle()+"' , @level0type=N'SCHEMA',@level0name=N'dbo', @level1type=N'TABLE',@level1name=N'"+entity.getTableName()+"';");
		}
		for(ColumnInfo a:entity.getColumns()){
	...
	private void formatterColumn(DBTable tableInfo){
		for(DBColumn columninfo : tableInfo.getColumns()){
			//将名称大写
